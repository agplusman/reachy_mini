...
class ReSpeaker:
    ...
    def read_values(self, name: str) -> AudioParameterValues | None:
        ...
        return _decode_parameter_values(name, raw_values)
    ...
    def apply_audio_config(
        ...
        *,
        ...
        for name, values in config:
            ...
            try:
                ...
                if verify:
                    ...
                    if not _values_match(actual_values, expected_values):
                        failures += 1
                        logger.warning(
                            "Audio parameter verification failed for %s: expected %s, got %s",
                            name,
                            _format_values(expected_values),
        ...
        logger.info("Applied Reachy Mini audio config: %s", _format_config(config))
        return True
...
def _decode_int32_values(
    raw_values: object, value_count: int, *, signed: bool
) -> tuple[int, ...] | None:
    if not isinstance(raw_values, Sequence):
        return None
...
def _format_config(config: AudioConfig) -> str:
